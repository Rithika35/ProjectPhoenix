...
float  h_0 = 16; //Launch site elevation above sea level (2000ft / 609.6m for FAR launch site)/ orlanod is 25 m(82ft)
...
const int consecutiveReadings = 7; // number of readings needed to exit lockout
...
int DrainTime = 5000000;//seconds to drain ballast 45 seconds
int TurnOffValve = 0;
...
float DroneDeployment = 7 ;//altitude of drone deployment in meters
...
void loop() {
  ...
  if (time - lastReadingTime >= readingInterval) {
    ...
    if ((liftoff == 1) && (lockout == 1) ){
      ...
      if(time-LiftOffTime > SuperSonicCount){//if simulated lockout period is over  
        unsigned long LockoutOver = time-LiftOffTime;
        int Lo = (LockoutOver/ 1000000) % 10;
        Serial.println(Lo);
    ...
    if(Apogee == 1 && liftoff==1 && lockout ==0){
      ...
      TurnOffValve = 1;
      unsigned long AfterApogee = time - ApogeeTime;
      if((TurnOffValve == 1) && (DrainTime < (AfterApogee))){
        int solenoidtime = (AfterApogee/ 1000000) % 10;
